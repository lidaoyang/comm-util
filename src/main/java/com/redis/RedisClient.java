...
public class RedisClient {
	...
	private static String PASSWORD= "e260e7bf521s";
	/**
	...
	private static synchronized void poolInit() {
		if (RWPool == null)
			RWPool = createJedisPool(PORT, PASSWORD);
//			RWPool = createJedisPool("127.0.0.1", null);
	...
	public static Jedis getReadyOnlyJedis() {
		Jedis jedis =null;
		if (RPool==null) {
			RPool = createJedisPool(PORT, PASSWORD);
...
//				Set<String> set_keys = jedis.keys(pattern);
	...
	public static int setex(String key,String value,int seconds) {
		Jedis jedis = getJedis();
		int ret =0;
		if (jedis != null) {
			try {
	...
	public static void main(String[] args) {
		Jedis jedis = getJedis();
		try {
			System.out.println(jedis.get("trigger_stocks"));
